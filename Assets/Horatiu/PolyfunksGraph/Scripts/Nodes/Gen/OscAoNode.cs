namespace Polyfunks
{
    ...
    public class OscAoNode : Node, IPolyfunksTick
    {
        [Input] public double freq = 220;
        [Input] public double phasePi = 0;

        [Input] public double ampl = 1;
        [Input] public double offset = 0;
        ...
        void IPolyfunksTick.Tick(double deltaTime)
        {
            ...
            _t += deltaTime * _freq * MathUtils.TAU;
            
            if (double.IsNaN(_t))
                _t = 0;

            // do the calculation in the tick, so we save performance when a lot of nodes ask for the value.
            var _phase = GetInputValueDouble(nameof(phasePi), this.phasePi) * MathUtils.PI;
            var _ampl = GetInputValueDouble(nameof(ampl), this.ampl);
            var _offset = GetInputValueDouble(nameof(offset), this.offset);

            _oscOut = Math.Sin(_t + _phase) * _ampl + _offset;
