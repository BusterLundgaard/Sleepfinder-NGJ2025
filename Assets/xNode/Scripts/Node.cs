...
#region Inputs/Outputs
        ...
        /// BEWARE! it creates garbage when using double, float, etc. In that case, use GetInputValueDouble.
        /// </summary>
        ...
        // HHH: this version of GetInputValue is necessary for audio processing because doubles get boxed into objects which creates 24B of garbage every time.
        //      That means for 44100 doubles per second, and there's gonna be a LOT of doubles each second when u have a bunch of nodes, so the GC would create a lot of crackles.
        // This method avoids the GC entirely, but you have to explicitly call GetInputValueDouble().
        public double GetInputValueDouble(string fieldName, double fallback = 0)
        {
            NodePort port = GetPort(fieldName);
            if (port != null && port.IsConnected) return port.GetInputValueDouble();
            else return fallback;
        }
        ...
        /// This version returns double avoiding the use of object, which causes 24B GC allocations for every double.
        /// </summary>
        public virtual double GetValueDouble(NodePort port)
        {
            Debug.LogWarning("No GetValue(NodePort port) override defined for " + GetType());
            return 0;
        }
